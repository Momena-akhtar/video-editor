...
export class ZoomEffectService {
  ...
  async applyZoomEffect(inputPath: string, outputPath: string, opts: ZoomOptions = {}): Promise<void> {
    ...
    // The key fix: use the correct zoompan parameters
    // d = duration in frames that each input frame should be shown
    // For a smooth zoom over the specified duration, we need d to equal totalFrames
    // and the zoom expression should be based on the current time or frame position
    
    // Use frame-based progress but with the correct total frame count
    // 'on' is the current output frame number (0-based)
    const frameProgress = `(on/${totalFrames-1})`;
    const clampedProgress = `min(${frameProgress}, 1)`;
    
    const easingExpr = this.easingExpression(clampedProgress, easing);
    ...
    // Key fix: set d to the total number of frames we want the zoom to last
    // This means the first (and only) input frame will be stretched across all output frames
    const filterComplex = [
      `[0:v]zoompan=`,
      `z='${zoomExpr}':`,
      `x='${xExpr}':`,
  ...
  private easingExpression(p: string, easing: Easing): string {
    switch (easing) {
      case "linear": 
        return p;
      case "ease-in": 

...
export class ZoomEffectService {
  ...
  async applyZoomEffect(inputPath: string, outputPath: string, opts: ZoomOptions = {}): Promise<void> {
    ...
    console.log(`Zoom config: ${startZoom} -> ${endZoom} over ${durationSec}s`);

    // Use a different approach: scale filter with time-based interpolation
    // This is more reliable than zoompan for this specific use case
    
    // Calculate progress based on time
    const timeProgress = `min(t/${durationSec}, 1)`;
    const easingExpr = this.easingExpression(timeProgress, easing);
    const scaleValue = `${startZoom} + (${endZoom} - ${startZoom}) * ${easingExpr}`;
    
    // Use scale filter with proper centering
    const filterComplex = [
      `[0:v]scale=`,
      `iw*${scaleValue}:`,
      `ih*${scaleValue}:`,
      `flags=lanczos,`,
      `crop=${probe.width}:${probe.height}:`,
      `(iw-${probe.width})/2:`,
      `(ih-${probe.height})/2[zoomed]`
    ].join('');
